...
// Modified to add support for "ws" and "wss" schemes.
// Modified to add GetHost(), GetQuery(), and GetHostAndQuery() methods.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

using System;

namespace Easemob.Common.Net
{
	/// <summary>
	...
	public class UriEx
	{
		...
		protected void ParseUriString(ref string uriString)
		{
			...
			if (schemeLower == "http" || schemeLower == "https" || schemeLower == "ws" || schemeLower == "wss")
			{
				if (uriString.Substring(startIndex).IndexOf("//") == 0)
				{
					...
					else
					{
						...
						switch (schemeLower)
						{
							case "http":
							case "ws":
								m_port = HttpDefaultPort; 
								break;
							case "https":
							case "wss":
								m_port = HttpsDefaultPort; 
								break;
						}
					}
			...
			{   // If absolute Uri was passed - create Uri object.
				switch (uriKind)
				{
					case UriKind.Absolute:
						{
							UriEx testUri = new UriEx(uriString);
					...
					case UriKind.Relative:
						{
							UriEx testUri = new UriEx(uriString, UriKind.Relative);
							if (!testUri.IsAbsoluteUri)
							{
								return true;
							}
		...
		/// </summary>
		/// <remarks>
		/// Based on RFC3986 Generic Syntax
		///	  authority = //[userinfo@]host[:port]
		///	  scheme:[authority][/path]["?" query]
		///	  http:[//[username:password@]www.test.com[:80]][/folder/file][?a=b&c=d]
		/// </remarks>
		public string GetPath()
		{
			int posAuth = this.AbsoluteUri.IndexOf("//");
			int posPath = (posAuth >= 0 ? this.AbsoluteUri.IndexOf('/', posAuth + 2) : this.AbsoluteUri.IndexOf('/'));
			int posQuery = this.AbsoluteUri.IndexOf('?');
			int lenUri = this.AbsoluteUri.Length;
			int lenPath = (posPath >= 0 ? (posQuery >= 0 ? posQuery - posPath : lenUri - posPath) : 0);

			return (lenPath > 0 ? this.AbsoluteUri.Substring(posPath, lenPath) : "/");
		}
		...
		public string GetQuery()
		{
			...
			return (posQuery >= 0 ? this.AbsoluteUri.Substring(posQuery) : "");
		}
		...
		public string GetPathAndQuery()
		{
			...
			string path = (lenPath > 0 ? this.AbsoluteUri.Substring(posPath, lenPath) : "/");
			return string.Concat(path, (posQuery >= 0 ? this.AbsoluteUri.Substring(posQuery) : ""));
		}
	}
}
